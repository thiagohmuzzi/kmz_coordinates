...
TOR_GRID = next((p for p in [ROOT / "TO27CSv1.gsb", THIS.parent / "TO27CSv1.gsb", Path.cwd() / "TO27CSv1.gsb"] if p.exists()), None)
ON_GRID  = next((p for p in [ROOT / "ON27CSv1.gsb", THIS.parent / "ON27CSv1.gsb", Path.cwd() / "ON27CSv1.gsb"] if p.exists()), None)

st.caption(
    "Input rows may contain either NAD83 Geographic (lat/long) or NAD27 / UTM Zone 17N (N/E). "
    "Toronto grid (TO27CSv1.gsb) is applied by default for UTM rows; if a point is outside that grid, "
    "Ontario grid (ON27CSv1.gsb) is used as fallback."
)

# Ensure PROJ sees grids
...
if st.button("Convert"):
    try:
        ...
        # Column selection (allow synonyms)
        def pick(opts):
            for o in opts:
                if o in df.columns: return o
            return None

        col_folder = pick(["folder","group","layer"])
        col_name   = pick(["feature_name","name","label","id","title"])
        col_lat    = pick(["lat","latitude"])
        col_lon    = pick(["long","lon","longitude"])
        col_n      = pick(["n","northing","utm_n","utm_northing","y"])
        col_e      = pick(["e","easting","utm_e","utm_easting","x"])
        col_z      = pick(["elevation","elev","z","altitude","height","elevation_m"])
        if col_name is None:
        ...
        def transformer_for(grid_path: Path) -> Transformer:
            # NAD27/UTM17N -> NAD83 geographic (deg)
            pipe = (
                f"+proj=pipeline "
        ...
        # --------------- Build final lon/lat for every row ---------------
        ...
        # store original inputs for validation
        ...
        if col_n and col_e and tr_tor:
            ...
            if m_utm.any():
                lon_t, lat_t = tr_tor.transform(df.loc[m_utm, col_e].to_numpy(),
                ...
                bad = lon_t.isna() | lat_t.isna()
                if bad.any() and tr_on:
                    lon_o, lat_o = tr_on.transform(df.loc[bad, col_e].to_numpy(),
        ...
        if out.empty:
            st.error("No valid coordinates were found after processing lat/long and/or N/E."); st.stop()

        # Rounding for validation output
        ...
        def pm_point(name, lon, lat, elev=None):
            ...
            return ("      <Placemark>\n"
                    ...
                    "      </Placemark>\n")
        def pm_linestring(name, coords):  # coords: list of (lon,lat[,elev])
            if len(coords) >= 2 and (coords[0][0] != coords[-1][0] or coords[0][1] != coords[-1][1]):
                coords.append(coords[0])  # close polyline
        ...
        for folder_name, g_folder in out.groupby(folder_series):
            ...
            for feat, g_feat in g_folder.groupby(out["feature_name"]):
                ...
                coords = [(float(r["lon"]), float(r["lat"]), float(elev_series.loc[i])) if pd.notna(elev_series.loc[i]) else (float(r["lon"]), float(r["lat"])) for i, r in g_feat.iterrows()]
                if len(coords) >= 2:
        ...
        # KMZ packaging (only KMZ, as requested)
        kmz = BytesIO()
        ...
        with pd.ExcelWriter(xbuf, engine="openpyxl") as xw:
            ...
            hdr = pd.DataFrame({
                "Validation": [
                    "All coordinates exported in NAD83 Geographic (lat/lon).",
                    "UTM inputs interpreted as NAD27 / UTM Zone 17N; Toronto grid default with Ontario fallback per-row.",
                ]
            })
            hdr.to_excel(xw, index=False, sheet_name="README")
        ...
        st.download_button("Download Validation Excel", data=xbuf.getvalue(),
