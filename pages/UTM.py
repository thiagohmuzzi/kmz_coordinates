...
def build_excel_template() -> bytes:
    ...
    for cell_ref, fill in [("B1", geo_fill), ("D1", utm_fill)]:
        cell = ws[cell_ref]
    ...
    for row in range(3, 1003):
        ws[f"B{row}"].number_format = "0.000000000"  # lat
        ws[f"C{row}"].number_format = "0.000000000"  # long
        ws[f"D{row}"].number_format = "0.000"        # E
        ws[f"E{row}"].number_format = "0.000"        # N
...
def build_output_excel(out: pd.DataFrame) -> bytes:
    ...
    for row_idx, record in enumerate(out.itertuples(index=False), start=3):
        ws.cell(row=row_idx, column=1).value = blank_if_nan(record.feature_name)
        ws.cell(row=row_idx, column=2).value = blank_if_nan(record.lat)
        ws.cell(row=row_idx, column=3).value = blank_if_nan(record.long)
        ws.cell(row=row_idx, column=4).value = blank_if_nan(record.E)
        ws.cell(row=row_idx, column=5).value = blank_if_nan(record.N)
    ...
    for row in range(3, ws.max_row + 1):
        ws[f"B{row}"].number_format = "0.000000000"
...
if convert_clicked and up:
    try:
        ...
        # Numeric coercion
        for c in [col_lat, col_long, col_e, col_n]:
